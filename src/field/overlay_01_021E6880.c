...
void FieldInput_Update(FieldInput *input, FieldSystem *fieldSystem, u16 newKeys, u16 heldKeys) {
    ...
    u32 avatar14 = PlayerAvatar_GetUnk14(fieldSystem->playerAvatar);
    u32 avatar10 = PlayerAvatar_GetUnk10(fieldSystem->playerAvatar);
...
static u32 GetInteractedMetatileScript(FieldSystem *fieldSystem, u8 metatile) {
    u32 direction = PlayerAvatar_GetFacingDirection(fieldSystem->playerAvatar);
    u32 standingMetatile = GetMetatileBehavior(fieldSystem, PlayerAvatar_GetXCoord(fieldSystem->playerAvatar), PlayerAvatar_GetZCoord(fieldSystem->playerAvatar));
